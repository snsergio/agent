...
# -*- coding: utf-8 -*-
#######################################################################################################################
versao = "metricserver-v4.20-PUB-4d87eb0-20230508150250"
#######################################################################################################################
import logging
import time
...
class server_exec:
    ...
    def collect_server(getServer):
        ...
        if serverout != "": serverMetrics = server_exec.server_metrics(serverout)
        else: serverMetrics = 0
        return serverMetrics
        #----------------------------------------------------------------------------------------------------------------------
    def server_metrics(osData):
        import os
        resposta = {
            "cpuCores": 0,
            "servertime": 0,
        ...
        try: resposta["servertime"] = osData[0].split("up", 1)[0].strip()[-8:]
        except: 
        ...
        getMem = c.exec_cmd("free", c.debugMode)["output"].splitlines()[1].split(":")[1].split()
