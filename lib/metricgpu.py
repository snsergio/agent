...
# -*- coding: utf-8 -*-
#######################################################################################################################
versao = "metricgpu-v4.20-PUB-4d87eb0-20230508150250"
#######################################################################################################################
import logging
import time
...
class gpu_exec:
    ...
    def collect_gpu(getGpu):
        ...
        if getGpu:
            ...
            try: gpuout = c.exec_cmd(["nvidia-smi", "-x", "-q"], c.debugMode, "xml")["output"]["nvidia_smi_log"]
            except: 
        ...
        if gpuout != "": gpuMetrics = gpu_exec.gpu_metrics(gpuout, c.logFirstRun)
        else: gpuMetrics = 0
        return gpuMetrics
        #----------------------------------------------------------------------------------------------------------------------
    def gpu_metrics(gpuData, logFirstRun):
        import os
        resposta = {
            "card": "",
            "driver": "",
