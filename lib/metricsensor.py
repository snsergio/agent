...
# -*- coding: utf-8 -*-
#######################################################################################################################
versao = "metricsensor-v4.20-PUB-4d87eb0-20230508150250"
#######################################################################################################################
import logging
import time
...
class sensor_exec:
    ...
    def collect_sensor(getSensor):
        ...
        if sensorout != "": sensorMetrics = sensor_exec.sensor_metrics(sensorout, c.logFirstRun)
        else: sensorMetrics = 0
        return sensorMetrics
        #----------------------------------------------------------------------------------------------------------------------
    def sensor_metrics(sensorOut, logFirstRun):
        resposta = {
        ...
        for item in range(len(sensorOut)):
            if ("CPU" in sensorOut[item] and "Temp" in sensorOut[item]) or "Package" in sensorOut[item] or "Core" in sensorOut[item]:
                try:
                    ...
                    cpuTemp = 0.0
                    tempMax = 0.0
                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in CPU Temperature")
                finally:
                ...
                while (len(sensorOut) < (item + contador)) or (len(sensorOut[item + contador].strip()) > 0):
                        ...
                        if ":" in sensorOut[item + contador].strip():
                            ...
                            if "(" in pciTemp: 
                                try: pciTemp = float(pciTemp.strip().split("(")[0].strip()[:-2].replace(',', '.').replace("+", ""))
                                    ...
                                    pciTemp = -1
                                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in PCI Temperature (1)")
                            else:
                                try: pciTemp = float(pciTemp.strip()[:-2].replace(',', '.').replace("+", ""))
                                except:
                                    pciTemp = -1
                                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in PCI Temperature (2)")
                    ...
                    if "power" in sensorOut[item + contador]:
                        ...
                        try: resposta["pciPower"] = float(pciPwr[1].split("(")[0].strip()[:-1])
                            ...
                            if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in PCI Power")
                        try: 
                            a = pciPwr[1].split("(")[1].strip()
                            if "crit" in a: resposta["pciPowerMax"] = float(a.split("=")[1][:-2])
                        except: 
                            if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in PCI Power Max")
                    contador += 1    
                ...
                while (len(sensorOut) < (item + contador)) or (len(sensorOut[item + contador].strip()) > 0):
                    if "temp" in sensorOut[item + contador]:
                        try: 
                            ...
                            mbTemp = -1
                            if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in MB Temperature (1)")
            ...
            if "Vcore" in sensorOut[item]:
                ...
                try: 
                    ...
                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in vCore voltage")
            if "3.3V" in sensorOut[item]:
                try: resposta["v3.3"] = float(sensorOut[7].split(":")[1].strip().split("V")[0].strip().replace(',', '.'))
                except: 
                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in v3.3 voltage")
            if "5V" in sensorOut[item]:
                try: resposta["v5.0"] = float(sensorOut[8].split(":")[1].strip().split("V")[0].strip().replace(',', '.'))
                except: 
                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in v5.0 voltage")
            if "12V" in sensorOut[item]:
                try: resposta["v12.0"] = float(sensorOut[9].split(":")[1].strip().split("V")[0].strip().replace(',', '.'))
                except: 
                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in v12.0 voltage")
            if "CPU Fan" in sensorOut[item]:
                try: resposta["cpuFanRPM"] = int(sensorOut[10].split(":")[1].strip().split("RPM")[0].strip().replace(',', '.'))
                except: 
                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in CPU Fan RPM")
            if "Chassis Fan" in sensorOut[item]:
                try: resposta["chassisFanRPM"] = int(sensorOut[11].split(":")[1].strip().split("RPM")[0].strip().replace(',', '.'))
                except: 
                    if not logFirstRun: logging.warning(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-sensor_exec.sensor_metrics: Error in Chassis Fan RPM")
