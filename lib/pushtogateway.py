...
# -*- coding: utf-8 -*-
#######################################################################################################################
versao = "pushtogateway-v5.02-PUB-f0875b2-20230703130653"
#######################################################################################################################
import logging
import time
...
class push_data:
    ...
    def set_data(self):
        ...
        if self.configDict["getJetson"] >= 1:
            if self.metricDict["jetsonMetrics"]["jetsonExecError"] or self.metricDict["jetsonMetrics"]["jtopExecError"]: self.jetsonExecError.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], mod="gpu_arm").set(1)
    ...
    def push_to_gateway(self):
        ...
        jobId = str(self.configDict["customerName"][:7] if len(self.configDict["customerName"]) > 7 else self.configDict["customerName"]) + "_" + \
            ...
            str(self.configDict["hostName"][:7] if len(self.configDict["hostName"]) > 7 else self.configDict["hostName"])
