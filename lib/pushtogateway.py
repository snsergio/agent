...
class push_data:
    ...
    def __init__(self, configDict, metricDict):
        ...
        if configDict["getBackup"] >= 1:
            self.bkpStatus = Gauge("backup_status", "Check backup data returned no errors", ["customer", "station", "host", "path", "name", "type"], registry=self.registry)
            self.bkpAccess = Gauge("backup_access", "Backup file last accessed", ["customer", "station", "host", "path", "name", "type"], registry=self.registry)
            self.bkpModified = Gauge("backup_modified", "Backup file last modified", ["customer", "station", "host", "path", "name", "type"], registry=self.registry)
            self.bkpExpire = Gauge("backup_expire", "Backup file age based on frequency", ["customer", "station", "host", "path", "name", "type"], registry=self.registry)
        # Metric Cam
        ...
        if configDict["getDocker"] == 2:
            self.dockerPorts = Info("docker_ports", "Docker Image Ports Info", ["customer", "station", "host", "image", "command", "names"], registry=self.registry)
        if configDict["getDocker"] >= 1:
            self.dockerStatusInfo = Info("docker_status_info", "Docker Image Current Status", ["customer", "station", "host", "image", "command", "names"], registry=self.registry)
            self.dockerStatus = Gauge("docker_status", "Docker Image Current Status", ["customer", "station", "host", "image", "command", "names"], registry=self.registry)
            self.dockerStatusAge = Gauge("docker_status_age", "Docker Image Current Status Age", ["customer", "station", "host", "image", "command", "names"], registry=self.registry)
            self.dockerCreated = Gauge("docker_created", "Docker Image Created Age", ["customer", "station", "host", "image", "command", "names"], registry=self.registry)
        # Metric Nvidia GPU
        ...
        if configDict["getMonOsProc"] >= 1:
            self.processCPU = Gauge("process_cpu", "Running Process CPU Usage", ["customer", "station", "host", "process", "proccmd"], registry=self.registry)
            self.processMEM = Gauge("process_mem", "Running Process Memory Usage", ["customer", "station", "host", "process", "proccmd"], registry=self.registry)
            self.processPID = Gauge("process_pid", "Running Process PID", ["customer", "station", "host", "process", "proccmd"], registry=self.registry)
            self.processTime = Gauge("process_time", "Running Process CPU time used (1/100s)", ["customer", "station", "host", "process", "proccmd"], registry=self.registry)
            self.processActive = Gauge("process_active", "Selected Processes Running Status", ["customer", "station", "host", "process", "proccmd"], registry=self.registry)
            self.processMissing = Gauge("process_missing", "Selected Processes Running Status", ["customer", "station", "host", "process"], registry=self.registry)
        # Metric Network
        ...
        if configDict["getRemoteOpen"] >= 1:
            self.remoteOpenStatus = Gauge("remote_open_status", "Remote port open status", ["customer", "station", "host", "remoteurl"], registry=self.registry)
            self.remoteOpenResponse = Info("remote_open_response", "Remote port open response", ["customer", "station", "host", "remoteurl"], registry=self.registry)
        # Metric Self IP
        if configDict["getSelfIp"] >= 1:
            self.selfIpPresent = Gauge("selfip_exec_time", "Ping time execution", ["customer", "station", "host", "ipaddr"], registry=self.registry)
        ...
        if configDict["getServer"] >= 1:
            ...
            self.serverTimeStamp = Info("station_timestamp", "Station Timestamp", ["customer", "station", "host"], registry=self.registry)
        ...
        if configDict["getTopProcess"] >= 1:
            self.topProcCPU = Gauge("top_process_cpu", "Running Top Offender Process CPU Usage", ["customer", "station", "host", "rank", "filter"], registry=self.registry)
    ...
    def set_data(self):
        # Metric API Get
        if self.configDict["getApiGet"] >= 1:
            for element in range(len(self.metricDict["apiGetMetrics"])):
                if self.configDict["getApiGet"] == 2:
                ...
                self.apiGetResponse.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], url=self.metricDict["apiGetMetrics"][element]["apiGetUrl"]).info({"api_get_url": self.metricDict["apiGetMetrics"][element]["apiGetResponse"]})
        # Metric Backup
        if self.configDict["getBackup"] >= 1:
            for element in range(len(self.metricDict["bkpMetrics"])):
                self.bkpStatus.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], path=self.metricDict["bkpMetrics"][element]["path"], name=self.metricDict["bkpMetrics"][element]["name"], type=self.metricDict["bkpMetrics"][element]["type"]).set(self.metricDict["bkpMetrics"][element]["status"])
        ...
        if (self.configDict["getDocker"] >= 1) and (self.metricDict["dockerMetrics"] != 0):
            for element in range(len(self.metricDict["dockerMetrics"])):
                ...
                if self.configDict["getDocker"] == 2:
                    self.dockerPorts.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], image=imageInfo, command=self.metricDict["dockerMetrics"][element]["command"], names=self.metricDict["dockerMetrics"][element]["names"]).info({"docker_ports": portsInfo})
                self.dockerStatusInfo.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], image=imageInfo, command=self.metricDict["dockerMetrics"][element]["command"], names=self.metricDict["dockerMetrics"][element]["names"]).info({"docker_status_info": statusInfo})
                self.dockerStatus.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], image=imageInfo, command=self.metricDict["dockerMetrics"][element]["command"], names=self.metricDict["dockerMetrics"][element]["names"]).set(statusGauge)
                self.dockerStatusAge.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], image=imageInfo, command=self.metricDict["dockerMetrics"][element]["command"], names=self.metricDict["dockerMetrics"][element]["names"]).set(self.metricDict["dockerMetrics"][element]["statusTime"])
                self.dockerCreated.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], image=imageInfo, command=self.metricDict["dockerMetrics"][element]["command"], names=self.metricDict["dockerMetrics"][element]["names"]).set(self.metricDict["dockerMetrics"][element]["created"])
        # Metric Nvidia GPU
        if self.configDict["getGpuNvidia"] >= 1:
            for element in range(len(self.metricDict["gpuMetrics"])):
                if self.configDict["getGpuNvidia"] == 2:
        ...
        # Metric Monitored OS Processes
        if self.configDict["getMonOsProc"] >= 1:
            for element in range(len(self.metricDict["monOsProcMetrics"]) - 1):
                self.processTime.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], process=self.metricDict["monOsProcMetrics"][element]["processCfg"], proccmd=self.metricDict["monOsProcMetrics"][element]["cmd"]).set(self.metricDict["monOsProcMetrics"][element]["time"])
                self.processCPU.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], process=self.metricDict["monOsProcMetrics"][element]["processCfg"], proccmd=self.metricDict["monOsProcMetrics"][element]["cmd"]).set(self.metricDict["monOsProcMetrics"][element]["cpu"])
                self.processMEM.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], process=self.metricDict["monOsProcMetrics"][element]["processCfg"], proccmd=self.metricDict["monOsProcMetrics"][element]["cmd"]).set(self.metricDict["monOsProcMetrics"][element]["mem"])
                self.processPID.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], process=self.metricDict["monOsProcMetrics"][element]["processCfg"], proccmd=self.metricDict["monOsProcMetrics"][element]["cmd"]).set(self.metricDict["monOsProcMetrics"][element]["pid"])
                self.processActive.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], process=self.metricDict["monOsProcMetrics"][element]["processCfg"], proccmd=self.metricDict["monOsProcMetrics"][element]["cmd"]).set(self.metricDict["monOsProcMetrics"][element]["processActive"])
            self.processMissing.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], process=self.metricDict["monOsProcMetrics"][element]["processCfg"]).set(len(self.metricDict["monOsProcMetrics"]["missingProcess"]))
        # Metric Network
        if self.configDict["getNetwork"] >= 1:
        ...
        # Metric Remote Open Port
        if self.configDict["getRemoteOpen"] >= 1:
            for element in range(len(self.metricDict["remoteOpenMetrics"])):
                self.remoteOpenStatus.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], remoteurl=self.metricDict["remoteOpenMetrics"][element]["remoteOpenUrl"]).set(self.metricDict["remoteOpenMetrics"][element]["remoteOpenStatus"])
                self.remoteOpenResponse.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], remoteurl=self.metricDict["remoteOpenMetrics"][element]["remoteOpenUrl"]).info({"remote_open_response": self.metricDict["remoteOpenMetrics"][element]["remoteOpenResponse"]})
        # Metric Self IP
        if self.configDict["getSelfIp"] >= 1:
            for element in self.metricDict["selfIpMetrics"].keys():
                self.selfIpPresent.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], ipaddr=element).set(self.metricDict["selfIpMetrics"][element])
        # Metric Server Sensors
        if self.configDict["getSensor"] == 2:
        ...
        if self.configDict["getServer"] >= 1:
            ...
            self.serverTimeStamp.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"]).info({"station_timestamp": self.metricDict["serverMetrics"]["servertime"]})
        ...
        if self.configDict["getTopProcess"] >= 1:
            for element in range(len(self.metricDict["topMetrics"])):
                self.topProcCPU.labels(customer=self.configDict["customerName"], station=self.configDict["stationName"], host=self.configDict["hostName"], rank=self.metricDict["topMetrics"][element]["rank"], filter=str(self.metricDict["topMetrics"][element]["rank"])+"-"+self.configDict["hostName"]).set(self.metricDict["topMetrics"][element]["cpu"])
