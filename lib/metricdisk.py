...
# -*- coding: utf-8 -*-
#######################################################################################################################
versao = "metricdisk-v4.20-PUB-4d87eb0-20230508150250"
#######################################################################################################################
import logging
import time
...
class disk_exec:
    ...
    def collect_disk(getDisk):
        ...
        if getDisk:
            orderList = {}
            if not c.logFirstRun: logging.info(
                f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-metricdisk version: {versao}")
            try: diskdf = c.exec_cmd(["df"], c.debugMode)["output"].splitlines()
            except: 
                if not c.logFirstRun: logging.error(
                    f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-disk_exec.collect_disk: DF execution error")
    ...
    def disk_metrics(diskData, logFirstRun, debugMode):
        ...
        for unit in range(1, len(diskData["df"])):
            if "loop" not in diskData["df"][unit]:
                try: resposta[volNum] = {
