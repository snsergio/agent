...
# -*- coding: utf-8 -*-
#######################################################################################################################
versao = "metricbackup-v4.20-PUB-4d87eb0-20230508150250"
#######################################################################################################################
import logging
import time
...
class backup_exec: 
    ...
    def collect_backup(getBkp, bkpFolder, bkpFreq, bkpPrefix, bkpSuffix):
        resposta = {}
        if getBkp:
                    ...
                    if len(fileData) > 8:
                        ...
                        if checkPre and checkSuf:
                            try: fileStat = c.exec_cmd(["stat", statName], c.debugMode)["output"].splitlines()
                                ...
                                if not c.logFirstRun: logging.error(
                                    f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-backup_exec.collect_backup: stat file execution error")
                            backupout[item] = {
                                "name": fileData[-1], 
                                "path": bkpFolder,
                                "type": fileType, 
                                ...
                                "frequency": bkpFreq}
                            item += 1
        if len(backupout) > 0: return backup_exec.backup_metrics(backupout)
        else: return 0
        #----------------------------------------------------------------------------------------------------------------------
        ...
        if len(backupout) > 0:
            ...
            for element in range(len(backupout)):
                try: fileItem = {
                    ...
                    "access": time.time() - time.mktime(time.strptime(backupout[element]["access"][:19], "%Y-%m-%d %H:%M:%S")),
                    "modified": time.time() - time.mktime(time.strptime(backupout[element]["modified"][:19], "%Y-%m-%d %H:%M:%S")),
                    "age": time.time() - time.mktime(time.strptime(backupout[element]["modified"][:19], "%Y-%m-%d %H:%M:%S")) - 
                    ...
                    "modified": 0,
                    "age": 0}
                if fileItem["age"] < 0: fileItem["age"] = 0
