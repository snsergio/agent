...
# -*- coding: utf-8 -*-
#######################################################################################################################
versao = "metrictop-v4.20-PUB-4d87eb0-20230508150250"
#######################################################################################################################
import logging
import time
...
class top_exec:
    ...
    def collect_top(getTopProc, topCount):
        ...
        if topprocout != "": topProcMetrics = top_exec.top_proc_metrics(topprocout, topCount, c.debugMode)
        else: topProcMetrics = 0
        return topProcMetrics
        #----------------------------------------------------------------------------------------------------------------------
    def top_proc_metrics(topprocout, numProcs, debugMode):
        global logErrorFirstRun
        resposta, topProcCount = {}, 0
        if debugMode: logging.debug(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()))}-top_exec.top_proc_metrics: Process Qty: {numProcs}")
        if numProcs > 0:
